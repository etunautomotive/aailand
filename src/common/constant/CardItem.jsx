export const CardItem = [
  {
    img: "/ConversationPageLight.png",
    title: "SMS/Call",
    content:
      "Communication is literally the most important part of all sales processes. We've done the foundation right, providing a beautiful user experience and then bridged that with AI for automatic data collection and autonomous agent text.",
  },
  {
    img: "/BulkSMSLight.png",
    title: "Bulk SMS",
    content:
      "We've proven it with over 10 dealers: Bulk SMS outreach delivers response rates of 25-30% and conversion rates around 5%. With an average gross per copy of $4,000–$6,000, and dealers managing thousands of names at any time, this translates into hundreds of thousands in gross revenue—all driven by a single tool in our system. It's automation that doesn't just scale—it delivers measurable impact.",
  },
  {
    img: "/pipelinezoomlight.png",
    title: "Pipeline Management",
    content:
      "Our intelligent pipeline system automates lead progression based on real-time data and completed processes. AI-powered agents automatically move leads through stages as they complete key milestones, from initial contact to deal closure. The system analyzes interaction data, response patterns, and process completion to optimize the sales journey—all while maintaining full manual control when needed.",
  },
  {
    img: "/contactzoomlight.png",
    title: "Contact Management",
    content:
      "The Contact Center allows instant access to all leads ever entered in your system. It's snappy to pull even the oldest contacts and trigger them into one of multiple campaigns we have vertical agents trained on.",
  },
  {
    img: "/detailconwhitezoom.png",
    title: "Customer Cards",
    content:
      "The best salespeople spend time selling, not doing data entry. Yet the industry average is just 35% of time on direct selling. We've solved this by making customer data collection autonomous and instant. Every interaction automatically enriches the customer profile, letting salespeople focus on what matters: selling.",
  },
];

export const SolutionAllItem = [
  {
    img: "/ConversationPageLight.png",
    title: "SMS/Call",
    content:
      "Communication is literally the most important part of all sales processes. We've done the foundation right, providing a beautiful user experience and then bridged that with AI for automatic data collection and autonomous agent text.",
  },
  {
    img: "/BulkSMSLight.png",
    title: "Bulk SMS",
    content:
      "We've proven it with over 10 dealers: Bulk SMS outreach delivers response rates of 25-30% and conversion rates around 5%. With an average gross per copy of $4,000–$6,000, and dealers managing thousands of names at any time, this translates into hundreds of thousands in gross revenue—all driven by a single tool in our system. It's automation that doesn't just scale—it delivers measurable impact.",
  },
  {
    img: "/pipelinezoomlight.png",
    title: "Pipeline Management",
    content:
      "We build our pipeline to be simple. We provide access for full control of stages and properties. Easily configure your process—we just provide the foundation for you to do so.",
  },
  {
    img: "/contactzoomlight.png",
    title: "Contact Management",
    content:
      "The Contact Center allows instant access to all leads ever entered in your system. It's snappy to pull even the oldest contacts and trigger them into one of multiple campaigns we have vertical agents trained on.",
  },
  {
    img: "/detailconwhitezoom.png",
    title: "Customer Cards",
    content:
      "The best salespeople spend time selling, not doing data entry. Yet the industry average is just 35% of time on direct selling. We've solved this by making customer data collection autonomous and instant. Every interaction automatically enriches the customer profile, letting salespeople focus on what matters: selling.",
  },
  {
    img: "/pipelinepagedark.png",
    title: "Vertical Intelligence",
    content:
      "Built specifically for automotive sales, our platform embeds decades of industry wisdom into every feature. From inventory cycles to financing workflows, everything is calibrated for your success.",
  },
];

export const VerticalIntelligenceTitle =
  "Now, Our Foundation Powers Vertical Intelligence";

export const VerticalIntelligenceCards = [
  {
    title: "AAI Text",
    content:
      "AAI Text is well-trained on every process your dealership handles through text communication. From managing cold contacts to collecting trade-in pictures and documents, it leverages our foundation to ensure every step is seamless and efficient, handling everything from initial contact to credit pull authorization.",
  },
  {
    title: "AAI Call",
    content:
      "AAI Call is your calling co-pilot, serving as an invaluable helper in voice communications. It meticulously tracks calls, schedules follow-ups, and runs second confirmations to ensure settlement accuracy. Every conversation is monitored to maintain perfect information flow throughout the process.",
  },
  {
    title: "AAI Finance",
    content:
      "AAI Finance activates once AAI Text collects the necessary information for credit and lender submissions. Drawing from experience with hundreds of thousands of deals, it operates like a seasoned finance manager who knows every deal across your team and optimizes each submission.",
  },
  {
    title: "AAI Close",
    content:
      "AAI Close builds the ultimate strategy for every deal by analyzing credit information from AAI Finance. It generates tailored scripts for salespeople, enabling them to approach each conversation with the expertise and insights of seasoned finance managers for optimal outcomes.",
  },
];
